...
class GrabNewFileCommand(sublime_plugin.TextCommand):
    def run(self, edit, txt):
        for sel in self.view.sel():
            self.view.replace(edit, sel, txt)


class GrabGetCommand(sublime_plugin.TextCommand):
    result = None
    url = None
    location = None
    option = None
    ...
    def handle_threads(self, edit, threads, offset=0, i=0, dir=1):
        ...
        if status and self.option == 'package':
            ...
            new_file.run_command('grab_new_file', {'txt': txt})
